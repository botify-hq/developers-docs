...
For instance:
- `features.search_engines.orphans.crawls.google.total` is the total number of Google orphan URLs (crawled by Google but not by Botify).
- `features.search_engines.orphans.visits.google` is the total number of visits generated by Google orphan URLs.
